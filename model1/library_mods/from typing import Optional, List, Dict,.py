...
def _onnx_heatmaps_to_keypoints(
    ...
    x = (
        torch.tensor(0.5, dtype=torch.float32) + x_int.to(dtype=torch.float32)
    ) * width_correction.to(dtype=torch.float32)
    y = (
        torch.tensor(0.5, dtype=torch.float32) + y_int.to(dtype=torch.float32)
...
def _onnx_heatmaps_to_keypoints_loop(
    ...
    xy_preds = torch.zeros(
        (0, 3, int(num_keypoints)), dtype=torch.float32, device=maps.device
    )
    end_scores = torch.zeros(
        (0, int(num_keypoints)), dtype=torch.float32, device=maps.device
    )
    ...
    for i in range(int(rois.size(0))):
        ...
        xy_preds = torch.cat(
            (
                xy_preds.to(dtype=torch.float32),
        ...
        end_scores = torch.cat(
            (
                ...
                end_scores_i.to(dtype=torch.float32).unsqueeze(0),
            ),
            0,
        )
...
def heatmaps_to_keypoints(maps, rois):
    ...
    xy_preds = torch.zeros(
        (len(rois), 3, num_keypoints), dtype=torch.float32, device=maps.device
    )
    end_scores = torch.zeros(
        (len(rois), num_keypoints), dtype=torch.float32, device=maps.device
    )
...
class RoIHeads(nn.Module):
    ...
    def forward(
        ...
        # if self.training:
        # custom addon to calc loss during eval
        (
            proposals,
            matched_idxs,
            labels,
